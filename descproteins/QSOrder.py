...
def QSOrder(fastas, nlag=30, w=0.1, **kw):
    ...
    dataFile = re.sub('descproteins$', '', os.path.split(os.path.realpath(__file__))[0]) + r'\data\Schneider-Wrede.txt' if platform.system() == 'Windows' else re.sub('descproteins$', '', os.path.split(os.path.realpath(__file__))[0]) + r'/data/Schneider-Wrede.txt'
    dataFile1 = re.sub('descproteins$', '', os.path.split(os.path.realpath(__file__))[0]) + r'\data\Grantham.txt' if platform.system() == 'Windows' else re.sub('descproteins$', '', os.path.split(os.path.realpath(__file__))[0]) + r'/data/Grantham.txt'
...
if __name__ == '__main__':
    ...
    parser.add_argument("--lag", type=int, default=30, help="the lag value for QSOrderr descriptor")
    parser.add_argument("--weight", type=float, default=0.1, help="the weight value for QSOrder descriptor")
    ...
    encodings = QSOrder(fastas, nlag=args.lag, w=args.weight, **kw)
